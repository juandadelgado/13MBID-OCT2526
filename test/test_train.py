...
def test_train_model(tmp_path):
    # Se tiene que establecer la raiz del proyecto para acceder 
    # a los archivos de datos y métricas.
    project_root = Path(__file__).resolve().parents[1]

    baseline_path = project_root / "metrics" / "train_metrics.json"
    if  not baseline_path.exists():
        pytest.skip("Baseline metrics not found. Please run train_model.py to generate it.")
